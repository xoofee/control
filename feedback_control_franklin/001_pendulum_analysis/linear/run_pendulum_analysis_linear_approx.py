#!/usr/bin/env python3
"""Run the linear-approximation pendulum analysis in MATLAB."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run pendulum_analysis_linear_approx.m in MATLAB."
    )
...
def main() -> None:
    ...
    try:
        ...
        _, info = eng.pendulum_analysis_linear_approx(
            args.mass, args.length, args.gravity, args.torque, nargout=2
        )
        print(
            "Step angle range: "
            f"[{float(info['step_angle_min_rad']):.6g}, "
            f"{float(info['step_angle_max_rad']):.6g}] rad; "
            f"[{float(info['step_angle_min_deg']):.6g}, "
            f"{float(info['step_angle_max_deg']):.6g}] deg"
        )
    finally:
